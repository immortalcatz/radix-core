...
public final class LogicHelper
{
	...
	public static Point3D getNearbyBlockTopBottom(Entity entity, Block block, int maxDistanceAway)
	{
		...
		while (true)
		{
			Block currentBlock = entity.worldObj.getBlock(x + xMov, y + yMov, z + zMov);
	...
	public static List<Point3D> getNearbyFarmableLand(Entity entity, int startCoordinatesX, int startCoordinatesY, int startCoordinatesZ, int areaX, int areaZ)
	{
		...
		while (true)
		{
			Block block = entity.worldObj.getBlock(x + xMov, y + yMov, z + zMov);
	...
	public static List<Point3D> getNearbyHarvestableCrops(Entity entity, int startCoordinatesX, int startCoordinatesY, int startCoordinatesZ, int radius)
	{
		...
		while (true)
		{
			...
			if (currentBlock == Blocks.melon_block || currentBlock == Blocks.potatoes || currentBlock == Blocks.carrots || currentBlock == Blocks.pumpkin || currentBlock == Blocks.wheat || currentBlock == Blocks.reeds)
			{
				if (currentBlock == Blocks.wheat || currentBlock == Blocks.carrots || currentBlock == Blocks.potatoes)
				{
				...
				{
					if (currentBlock == Blocks.reeds)
					{
						//Check for reeds above the base reed.
						if (entity.worldObj.getBlock(startCoordinatesX + xMov, startCoordinatesY + yMov + 1, startCoordinatesZ + zMov) == Blocks.reeds)
						{
	...
	{
		float deltaX =  (float)(player.posX - entity.posX);
		float deltaY = (float)(player.posY - entity.posY);
		float deltaZ = (float)(player.posZ - entity.posZ);
